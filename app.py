...
# Tenta ler o arquivo CSV com opções para lidar com erros
try:
    # Lê o CSV, pulando linhas problemáticas temporariamente
        ...
        if op == 'Titulo das colunas':
            ...
            if view == 'Tabela interativa':
                ...
                for row in rows:
                    ...
                    if st.session_state.selected_column in row['Nome da Coluna'].values:
                        col_name = st.session_state.selected_column
                        col_data = columns_df[columns_df['Nome da Coluna'] == col_name].iloc[0]
    ...
    # Adicione outras funções para as opções 'Filtro de busca', 'Ocorrências', etc., conforme necessário

except pd.errors.ParserError as e:
    st.error(f"Erro ao ler o arquivo CSV: {e}")
    # Depuração: imprime e salva o cabeçalho e a linha problemática
    try:
        with open(csv_file, 'r', encoding='latin1') as file:
            ...
            # Salva em um arquivo de log para análise
        ...
        # Tenta ler apenas a linha 24 como fallback
        try:
            with open(csv_file, 'r', encoding='latin1') as file:
